...
complete -c inotifywait -l timefmt -d 'Set a time format string as accepted by  strftime (3) for use with the `%T\' c…'
...
complete -c inotifywait -s I -l inotify -d 'Watch using inotify (default for inotifywait)'
complete -c inotifywait -s F -l fanotify -d 'Watch using fanotify (default for fsnotifywait)'
