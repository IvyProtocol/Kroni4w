...
complete -c rofi -o info -d 'The info option shows useful information to include when reporting an issue'
...
complete -c rofi -o case-smart -d 'Start in case-smart mode behave like vim[cq]s smartcase, which determines cas…'
...
complete -c rofi -o drun-exclude-categories -d 'Exclude desktop files that are present in the listed categories'
...
complete -c rofi -s 5
complete -c rofi -o theme -d 'Path to theme file, or name of an installed theme'
...
complete -c rofi -o global-kb -o no-global-kb -d '(wayland) Override the compositor[cq]s keybindings, so that rofi can re-use t…'
